...
-- Рассчитать сессию на ставках по итогу 4 пулек.
-- p_totals = {"yandex_id": суммарные_очки_за_4_пульки, ...}
-- Банк = stake_per_pulka × 4 × 4 игрока. Награда по месту: ×2 / ×1.5 / ×0.5 / 0
-- от внесённой суммы одного игрока (stake_per_pulka × 4) — банк расходится
-- ровно полностью (2+1.5+0.5+0 = 4), монеты не создаются и не исчезают.
...
-- ============================================================
-- Правила дисконнекта/тайм-аута (реализуются на сервере игры,
-- эта схема лишь хранит нужные для этого поля):
-- - На ход даётся 20 секунд. Не походил вовремя — сервер играет
--   за игрока сам (простейший легальный ход), пока игрок не вернётся.
-- - Если игрок вышел или не отвечает — table_players.is_bot = true,
--   и это место доигрывает бот до конца сессии (все 4 пульки),
--   даже если игрок так и не вернулся. Ставка (fn_charge_stake_entry)
--   уже списана при входе, поэтому итоговый выигрыш/проигрыш всё
--   равно относится к вышедшему игроку — уйти от проигрыша выходом нельзя.
-- - table_players.last_seen_at обновляется клиентом на каждый ход/пинг,
--   чтобы сервер понимал, когда включать бота.
-- ============================================================
